...
module.exports = async (user, mailType) => {
 ...
 let emailContent,mailOptions
        if (mailType == 'verifyemail') {
             emailContent = `<div><h1>Please Click on The Below Link to verify your email adress</h1> <a href="http://localhost:3000/verifyemail/${enryptedToken}">${enryptedToken}</a> </div>`
             ...
             emailContent = `<div><h1>Please Click on The Below Link to reset your password</h1> <a href="http://localhost:3000/resetpassword/${enryptedToken}">${enryptedToken}</a> </div>`
             mailOptions = {
                from: "bahaeddinetlili9@gmail.com",
                to: user.email,
                subject: "Reset  password For Mon Auth",
                html: emailContent,
            }
